...
def load_secrets_from_vault() -> dict:
    ...
    required = ["client_id", "client_secret", "gmail_user", "gmail_app_password", "notify_to"]
    for r in required:
        if r not in secrets or not secrets[r]:
            raise RuntimeError(f"Missing '{r}' in decrypted vault data")

    # normalize notify_to to a list
    notify = secrets["notify_to"]
    if isinstance(notify, str):
        notify_list = [notify]
    elif isinstance(notify, list):
        notify_list = [x for x in notify if isinstance(x, str) and x.strip()]
    else:
        raise RuntimeError("notify_to in vault must be a string or list of strings")

    if not notify_list:
        raise RuntimeError("No valid notification recipients found in vault")

    secrets["notify_to"] = notify_list
    return secrets
...
GMAIL_APP_PASSWORD = _secrets["gmail_app_password"]
NOTIFY_TO: List[str] = _secrets["notify_to"]
...
def send_email(subject: str, body: str):
    ...
    if not GMAIL_USER or not GMAIL_APP_PASSWORD or not NOTIFY_TO:
        logging.error("Email credentials or recipients not configured.")
        return
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
            ...
            smtp.send_message(msg, from_addr=GMAIL_USER, to_addrs=NOTIFY_TO)
        logging.info("Email sent to %s: %s", NOTIFY_TO, subject)
    except Exception as e:
...
def send_notification(subject: str, body: str):
    """
    Sends a single notification email (e.g., for uncategorized, missing schedule, NAS issues, etc.).
    """
    logging.warning("NOTIFICATION: %s -- %s", subject, body.replace("\n", " | "))
    send_email(subject, body)


def send_run_summary(downloads: List[Dict]):
    """
    Sends an email summary at the end of each run with all downloads.
    ...
    if total == 0:
        body = "No new services were downloaded this run."
    else:
        lines = [f"Downloads this run: {total}", ""]
        for d in downloads:
            lines.append(f"- {d['name']} → {d['path']} (category: {d['category']})")
        body = "\n".join(lines)

    send_email("BoxCast Download Summary", body)
...
def ensure_nas_mounted() -> bool:
    ...
    - If still not mounted: send email alert and return False.
    """
...
def check_expected_schedule(token: str, state: Dict) -> Dict:
    ...
    If missing, send an email once per day.
    """
...
def weekly_analytics(token: str, state: Dict) -> Dict:
    ...
    send_email("Weekly BoxCast Summary", "\n".join(lines))
...
def main():
    logging.info("========== BoxCast Auto Downloader (Gmail) Started ==========")
    ...
    for b in broadcasts:
        ...
        # Christmas at Carbondale
            ...
            # Holiday services (stored under Special Services folder)
            holiday = detect_holiday(name_lower)
            if holiday:
    ...
    logging.info("========== BoxCast Auto Downloader (Gmail) Finished ==========")
